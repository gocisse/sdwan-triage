  ...
  'Cisco SD-WAN': {
    ...
    findings: {
      ...
      sdwan_control_down: fb(
        {
          cli: ['show sdwan control connections', 'show sdwan control local-properties', 'show sdwan omp peers', 'show sdwan certificate installed', 'show ip interface brief', 'show clock'],
          gui: ['vManage > Monitor > Network > [Device] > Control Status', 'vManage > Dashboard > Main Dashboard — check controller health', 'vManage > Administration > Settings > vBond — verify vBond address'],
          script: ['# REST API: Get all device control connections', 'curl -k -b cookies.txt "https://<vmanage>:8443/dataservice/device/control/connections?deviceId=<system-ip>"', '# REST API: Get controller status', 'curl -k -b cookies.txt "https://<vmanage>:8443/dataservice/device/control/networksummary"'],
        },
        {
          cli: ['! Verify vBond address is correct', 'show sdwan running-config | include vbond', '! Verify system IP and org name match vManage', 'show sdwan running-config | include system', '! Reset control connections (CAUTION)', 'request platform software sdwan control reset'],
          gui: ['vManage > Administration > Settings > vBond — verify IP address', 'vManage > Configuration > Certificates > Send to Controllers', 'vManage > Configuration > Devices > [Device] > ... > Reset Control Connections'],
        },
        {
          cli: ['show sdwan control connections  ! All should show "Up"', 'show sdwan omp peers  ! OMP peer state should be "Up"'],
          gui: ['vManage > Monitor > Network > [Device] > Control Status — all green'],
        },
        ['Control reset drops ALL tunnels — only use as last resort', 'Verify vBond, vSmart, and vManage are all reachable before resetting', 'Certificate issues are the #1 cause of control connection failures'],
        {
          documentationLinks: [{ title: 'Control Connection Troubleshooting', url: 'https://www.cisco.com/c/en/us/support/docs/routers/sd-wan/214509-troubleshoot-sd-wan-control-connections.html' }],
          tacNotes: ['Collect "show tech-support sdwan" and "show sdwan control local-properties" from the affected device', 'Check vManage audit log for recent certificate or policy changes'],
        },
      ),
    },
  },
