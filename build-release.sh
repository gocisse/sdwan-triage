...
CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build -o "${RELEASE_DIR}/sdwan-triage-darwin-amd64" ./${BUILD_DIR}
zip -j "${RELEASE_DIR}/sdwan-triage-darwin-amd64.zip" "${RELEASE_DIR}/sdwan-triage-darwin-amd64"

# Build for macOS (Apple Silicon)
echo "Building for macOS (arm64)..."
CGO_ENABLED=0 GOOS=darwin GOARCH=arm64 go build -o "${RELEASE_DIR}/sdwan-triage-darwin-arm64" ./${BUILD_DIR}
zip -j "${RELEASE_DIR}/sdwan-triage-darwin-arm64.zip" "${RELEASE_DIR}/sdwan-triage-darwin-arm64"

# Build for Linux (amd64)
echo "Building for Linux (amd64)..."
CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o "${RELEASE_DIR}/sdwan-triage-linux-amd64" ./${BUILD_DIR}
zip -j "${RELEASE_DIR}/sdwan-triage-linux-amd64.zip" "${RELEASE_DIR}/sdwan-triage-linux-amd64"

# Build for Linux (arm64)
echo "Building for Linux (arm64)..."
CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -o "${RELEASE_DIR}/sdwan-triage-linux-arm64" ./${BUILD_DIR}
zip -j "${RELEASE_DIR}/sdwan-triage-linux-arm64.zip" "${RELEASE_DIR}/sdwan-triage-linux-arm64"

# Build for Windows (amd64)
echo "Building for Windows (amd64)..."
CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -o "${RELEASE_DIR}/sdwan-triage-windows-amd64.exe" ./${BUILD_DIR}
